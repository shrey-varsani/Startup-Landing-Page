...
function drawWaves() {
  ...
  const primaryColor = isGerman ? 'rgba(185, 28, 2Red, 0.4)' : 'rgba(0, 112, 187, 0.35)';
  const primaryColor = isGerman ? 'rgba(185, 28, 28, 0.4)' : 'rgba(0, 112, 187, 0.35)';
  const secondaryColor = isGerman ? 'rgba(212, 160, 23, 0.2)' : 'rgba(139, 92, 246, 0.18)';

  // Draw 3 layers of overlaying curves
  const curves = [
    { amplitude: isHeroTalking ? 24 : 6, wavelength: 120, speed: 0.08, color: primaryColor, lineWidth: 2 },
    { amplitude: isHeroTalking ? 16 : 4, wavelength: 80, speed: -0.06, color: secondaryColor, lineWidth: 1.5 },
    { amplitude: isHeroTalking ? 30 : 8, wavelength: 160, speed: 0.04, color: isGerman ? 'rgba(212, 160, 23, 0.12)' : 'rgba(6, 182, 212, 0.15)', lineWidth: 1 }
  ];
...
export function renderPlaygroundWorkspace() {
  ...
  const currentCoach = COACHES_DB.find(c => c.id === selectedCoachId) || filtered[0];
  if (currentCoach) {
...
function selectCoach(coachId) {
  ...
  // Highlight visually
  const rows = document.querySelectorAll('#coaches-list-target .coach-item-row');
  rows.forEach((row) => {
...
function updateImmersiveClassroom(coach) {
  ...
  const promptContainer = document.getElementById('playground-prompts-container');
  ...
  if (details) details.innerText = lang === 'en' ? coach.accentEn : coach.accentDe;
  if (responseText) {
...
function executeClassroomExchange(prompt) {
  ...
  if (responseText) {
    responseText.innerHTML = getLanguage() === 'en'
...
export function initAnimations() {
  ...
  if (pmMicBtn) {
    pmMicBtn.addEventListener('click', () => {
      ...
      const firstPromptBtn = document.querySelector('#playground-prompts-scaffold .prompt-choice');
      if (firstPromptBtn) {
